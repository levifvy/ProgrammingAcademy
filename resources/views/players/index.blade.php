...
@section('content')
    <div class="flex justify-center bg-gradient-to-b from-purple-950 to-purple-500 h-screen">
        <div class="p-3 bg-transparent mt-8 mb-16">
            <div class="flex justify-center w-full mx-auto sm:w-2/3 md:w-1/2 lg:w-1/3">
                <div class="bg-gradient-to-br from-sky-500 to-gray-200 p-6 px-12 rounded">
                        ...
                        <tbody>
                            @foreach ($players as $key => $player)
                                <tr>
                                    ...
                                    <td class="border px-4 py-2">{{ $player->team ? $player->team->name : 'No team assigned' }}</td>
                                    <td class="border px-4 py-2">{{ $player->team && $player->team->category ? $player->team->category->name : 'No category assigned' }}</td>
                                    <td class="border px-4 py-2">
                                        <div class="flex item-center justify-center space-x-4">
                                            ...
                                            <form action="{{ route('players.destroy', $player) }}" method="POST" class="d-inline" title="delete">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-danger btn-sm" onclick="return confirm('Are you sure you want to delete this player?')">
