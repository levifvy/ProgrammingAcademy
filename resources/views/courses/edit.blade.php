        ...
        <div class="m-auto md:w-8/12 lg:w-6/12 xl:w-6/12">
            <div class="rounded-xl bg-gradient-to-br from-sky-500 to-gray-200 shadow-xl">
                <div class="p-6 sm:p-8">
                    <div class="space-y-4">
                        <h2 class="mb-8 text-2xl text-cyan-900 font-bold">Modify your course:</h2>
                    </div>
                    <form action="{{route('courses.update', $course)}}" method="POST" class="mx-5 my-5">
                        @csrf
                        @method('put')   
                        <label class="relative block p-3 border-2 border-gray-500 transition duration-300 hover:border-blue-400 focus:bg-blue-50 active:bg-blue-100 rounded" htmlFor="name">
                            ...
                            <input class="w-full bg-transparent p-0 text-sm  text-gray-500 focus:outline-none" id="name" type="text" name="name" value="{{old('name', $course->name)}}" placeholder="Please fill out this field" />
                            @error('name')
                                <br>
                                <small>*{{$message}}</small>
                                <br>
                        ...
                        <label class="relative block p-3 border-2 mt-5 border-gray-500 transition duration-300 hover:border-blue-400 focus:bg-blue-50 active:bg-blue-100 rounded" htmlFor="description">
                            ...
                            <textarea class="w-full   p-0 text-sm border-none bg-transparent text-gray-500 focus:outline-none" id="name" type="text" name="description" rows="5" placeholder="Tell us about your course.">{{old('description', $course->description)}}</textarea>
                            @error('description')
                                <br>
                                <small>*{{$message}}</small>
                                <br>
                        ...
                        <div class="flex justify-center">
                            <div class="w-1/2 flex justify-between">
                                <a href="{{route('courses.show', $course)}}" class="mt-5 border-2 px-5 py-2 rounded-lg border-gray-500 bg-gray-200 hover:bg-red-400 hover:border-red-400 border-b-4 font-black translate-y-2 border-l-4">Cancel</a>
