...
@section('content')
<div class="relative py-8 bg-gradient-to-br from-purple-950 to-white">
            ...
            <div class="m-auto md:w-8/12 lg:w-6/12 xl:w-6/12">
                <div class="rounded-xl bg-white shadow-xl">
                    <div class="bg-yellow-200 p-6 sm:p-8 rounded">
                        ...
                        <form action="{{ route('courses.store') }}" method="POST" class="mx-5 my-5">
                            @csrf
                            <h1 class="text-xl font-semibold mt-5">Category:</h1>
                            <p class="text-black text-sm font-normal flex gap gap-2 pt-2">
                                <div class="relative">
                                    <select class="group h-10 px-6 border-2 border-gray-300 rounded-lg transition duration-300 hover:border-blue-400 focus:bg-blue-50 active:bg-blue-100 w-full"
                                        name="category_id">
                            ...
                            <label class="relative block p-3 border-2 mt-5 border-gray-500 transition duration-300 hover:border-blue-400 focus:bg-blue-50 active:bg-blue-100 rounded"
                                ...
                                <textarea class="w-full   p-0 text-sm border-none bg-transparent text-gray-500 focus:outline-none" id="name"
                                    type="text" name="description" rows="5" placeholder="Tell us about your course.">{{ old('description') }}</textarea>
                                @error('description')
                                    <br>
                                    <small class="text-red-500">*{{ $message }}</small>
                                    <br>
